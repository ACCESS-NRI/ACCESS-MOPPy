...
class TestSupergrid:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("resolution,expected_file", [
        ("100 km", "mom1deg.nc"),
        ("25 km", "mom025deg.nc"),
        ("10 km", "mom01deg.nc"),
    ])
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("resolution,error_match", [
    ...
    def test_extract_grid_b_grid_all_types(self, supergrid_instance, grid_type):
        ...
        expected_keys = ["latitude", "longitude", "vertices_latitude", 
                         "vertices_longitude", "i", "j", "vertices"]
    ...
    def test_extract_grid_c_grid_all_types(self, supergrid_instance, grid_type, symmetric):
    ...
    @pytest.mark.unit
    def test_extract_grid_c_grid_symmetric_vs_asymmetric_dimensions(self, supergrid_instance):
    ...
    def test_extract_grid_unsupported_grid_type(self, supergrid_instance, arakawa, symmetric):
        """Test that unsupported grid type raises error."""
        with pytest.raises(ValueError, match="is not a supported grid_type"):
            supergrid_instance.extract_grid(grid_type="X", arakawa=arakawa, symmetric=symmetric)
    ...
    def test_extract_grid_output_structure(self, supergrid_instance):
        ...
        np.testing.assert_array_equal(grid_info["i"].values, np.arange(len(grid_info["i"])))
        np.testing.assert_array_equal(grid_info["j"].values, np.arange(len(grid_info["j"])))
        np.testing.assert_array_equal(grid_info["vertices"].values, np.array([0, 1, 2, 3]))