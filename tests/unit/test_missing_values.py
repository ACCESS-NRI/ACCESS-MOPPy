...
class TestCMIP6VocabularyMissingValues:
    ...
    def vocabulary_instance(self, mock_vocab_data, mock_table_data):
        ...
        with patch.object(CMIP6Vocabulary, '_load_controlled_vocab', return_value=mock_vocab_data), \
    ...
    def test_get_cmip_missing_value_variable_specific(self, mock_vocab_data, mock_table_data):
        ...
        with patch.object(CMIP6Vocabulary, '_load_controlled_vocab', return_value=mock_vocab_data), \
            ...
            assert missing_value == 9.96921e+36
    ...
    def test_standardize_missing_values_nan_replacement(self, vocabulary_instance):
        ...
        # Check data values
        expected_data = np.array([[1.0, 2.0, expected_missing], [4.0, expected_missing, 6.0]])
    ...
    def test_standardize_missing_values_convert_existing(self, vocabulary_instance):
        ...
        da = xr.DataArray(
            data,
            dims=['lat', 'lon'],
            attrs={'units': 'K', 'missing_value': old_missing, '_FillValue': old_missing}
        )
        
        # Standardize missing values with conversion
        result = vocabulary_instance.standardize_missing_values(da, convert_existing=True)
    ...
    def test_standardize_missing_values_no_conversion(self, vocabulary_instance):
        ...
        expected_missing = 1e20
        expected_data = np.array([[1.0, expected_missing, old_missing], [4.0, old_missing, 6.0]])
        np.testing.assert_array_equal(result.values, expected_data)
