...
ROOT_FOLDER = (
    "/g/data/p73/archive/CMIP7/ACCESS-ESM1-6/spinup/Dec25-PI-control/"
)
...
def get_monthly_ocean_files(
    ...
    if not compound_name or '.' not in compound_name:
        raise ValueError(f"Invalid compound_name format: {compound_name}. Expected 'table.variable' format.")
    ...
    for model_variable in model_variables:
        ...
        try:
            ...
            if not matching_files:
                print(f"No files found for model variable '{model_variable}' with pattern: {search_pattern}")
            else:
                print(f"Found {len(matching_files)} files for model variable '{model_variable}'")
