...
def cmorise_ocean(file_paths, compound_name, cmor_dataset_json, mip_table):
    ...
    with resources.files("access_mopper.cmor_tables").joinpath("CMIP6_grids.json").open("r") as file:
        grid_table_id = cmor.load_table("cmip6-cmor-tables/Tables/CMIP6_grids.json")
    cmor.set_table(grid_table_id)

    cmor_axes = [] 
    # Define CMOR axes
    yaxis_id = cmor.axis(table_entry="j_index", 
                         units="1", 
                         coord_vals=y, 
                         cell_bounds=y_bnds)
    xaxis_id = cmor.axis(table_entry="i_index", 
                         units="1", 
                         coord_vals=x, 
                         cell_bounds=x_bnds)

    grid_id = cmor.grid(axis_ids=np.array([yaxis_id, xaxis_id]), 
                        latitude=lat, 
                        longitude=lon, 
