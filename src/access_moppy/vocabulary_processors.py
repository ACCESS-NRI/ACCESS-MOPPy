...
class CMIP6Vocabulary:
    ...
    def _get_source(self) -> Dict[str, Any]:
        try:
            ...
            override = _CMIP6_TEMP_SOURCE_OVERRIDES.get(self.source_id)
            if override is not None:
                if self.source_id not in _CMIP6_TEMP_SOURCE_WARNED:
                    _CMIP6_TEMP_SOURCE_WARNED.add(self.source_id)
                    warnings.warn(
                        f"Using temporary CMIP6/CMIP6Plus controlled vocabulary override for "
                        f"source_id '{self.source_id}'. Remove this shim once the bundled "
                        f"CMIP6Plus CVs provide the official source/institution entry.",
                        UserWarning,
                        stacklevel=3,
                    )
                return override
    ...
    def get_parent_experiment_attrs(self) -> Dict[str, Any]:
        ...
        if (
            parent_attrs["parent_source_id"] not in self.vocab["source_id"]
            and parent_attrs["parent_source_id"] not in _CMIP6_TEMP_SOURCE_OVERRIDES
        ):
            raise ValueError(
    ...
    def get_required_global_attributes(self) -> Dict[str, Any]:
        ...
        attrs = {
            ...
            "forcing_index": np.int32(variant["forcing_index"]),
            ...
            "history": f"{now} ; CMORised by ACCESS-MOPPy {_access_moppy_version}",
            "initialization_index": np.int32(variant["initialization_index"]),
            ...
            "physics_index": np.int32(variant["physics_index"]),
            "product": self.cmip_table["Header"].get("product"),
            "realization_index": np.int32(variant["realization_index"]),
            ...
            "table_id": self.table,
            "title": f"{self.source_id} output prepared for {self.mip_era}",
...
class CMIP7Vocabulary:
    ...
    def _get_compound_mapping(self, filename: str, section: str) -> str:
        """Return an attribute keyed by the full CMIP7 compound name."""
        entry = _vocab_files(self.table_dir) / filename
        with as_file(entry) as path:
            with open(path, "r", encoding="utf-8") as f:
                mappings = json.load(f)[section]
    ...
    def _get_long_name_override(self) -> str:
        return self._get_compound_mapping(
            "CMIP7_long_name_overrides.json", "long_name_overrides"
        )
    ...
    def get_required_global_attributes(self) -> Dict[str, Any]:
        ...
        attrs = {
            ...
            "title": f"{self.source_id} output prepared for CMIP7",
