...
def calculate_areacella(nlat=145, nlon=192, earth_radius=6371000.0):
    ...
    """
    # Handle internal calculation calling pattern
    if isinstance(nlat, xr.Dataset):
        # Called as internal calculation: calculate_areacella(ds, **kwargs)
        # Extract parameters from kwargs
        nlat = kwargs.get('nlat', 145)
        nlon = kwargs.get('nlon', 192)  
        earth_radius = kwargs.get('earth_radius', 6371000.0)
        return_dataset = True
    else:
        # Normal calling pattern: calculate_areacella(nlat=145, nlon=192, ...)
